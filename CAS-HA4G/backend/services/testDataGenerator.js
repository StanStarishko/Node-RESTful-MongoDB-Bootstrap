...
class TestDataGenerator {
    static async generateTestData() {
        ...
        const existingEnrollment = await Enrollment.countDocuments();

        // Generate 30 staff members
        for (let i = existingStaff + 1; i <= existingStaff + 30; i++) {
            await Staff.create({
        ...
        for (let i = existingStudents + 1; i <= existingStudents + 30; i++) {
        ...
        // Generate 30 courses
        for (let i = existingCourses + 1; i <= existingCourses + 30; i++) {
            await Course.create({
        ...
        // Generate 30 enrollments
        for (let i = existingEnrollment + 1; i <= existingEnrollment + 30; i++) { 
            ...
            await Enrollment.create({
                ...
                enrollmentDate: this.generateRandomDate(2024, 2025)
            });
        }
    }
